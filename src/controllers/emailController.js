...
const resolveVars = (text, vars, customerData = {}) => {
    if (!text) return '';
    ...
    // Replace custom field variables (e.g., {{field_name}})
    // Match any {{variable}} pattern that hasn't been replaced yet
    const customFieldPattern = /\{\{([a-zA-Z_][a-zA-Z0-9_]*)\}\}/g;
    resolved = resolved.replace(customFieldPattern, (match, fieldName) => {
        // Return the customer data value if it exists, otherwise keep the placeholder
        return customerData[fieldName] !== undefined && customerData[fieldName] !== null 
            ? String(customerData[fieldName]) 
            : match;
    });
...
export const sendEmailToCustomer = async (req, res) => {
    ...
    try {
        ...
        // 2. Fetch customer (all fields to support custom field variables)
        ...
        // 4. Resolve variables (including custom fields)
        const vars = {
        ...
        const resolvedSubject = resolveVars(subject, vars, customer);
        const resolvedBody    = resolveVars(body, vars, customer);
...
// ─────────────────────────────────────────────────────────────────────────────
// GET /email/variables - Get available template variables including custom fields
// ─────────────────────────────────────────────────────────────────────────────
export const getEmailVariables = async (req, res) => {
    let connection;
    try {
        const pool = await connectDB();
        connection = await pool.getConnection();
        ...
        // Standard variables
        const standardVars = [
            { variable: '{{customer_name}}', description: 'Customer first name' },
            { variable: '{{customer_phone}}', description: 'Customer phone number' },
            { variable: '{{customer_email}}', description: 'Customer email address' },
            { variable: '{{agent_name}}', description: 'Agent/user name' },
        ...
        const [customFields] = await connection.query(
            `SELECT field_name, field_label, field_type 
             ...
             WHERE company_id = ? AND is_active = true AND is_system_field = false
             ORDER BY display_order`,
            [companyId]
        );
        
        const customVars = customFields.map(field => ({
            variable: `{{${field.field_name}}}`,
            description: `${field.field_label || field.field_name} (${field.field_type})`,
            isCustomField: true
        }));
        ...
        return res.json({
            ...
            data: {
                ...
                custom: customVars,
                all: [...standardVars, ...customVars]
            }
        });
        
    } catch (err) {
        console.error('getEmailVariables error:', err);
        return res.status(500).json({ success: false, message: 'Server error' });
    } finally {
        if (connection) connection.release();
    }
